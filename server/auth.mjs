...
const JWT_SECRET = process.env.JWT_SECRET || "hockey-app-secret-change-in-production";
const JWT_EXPIRES_IN = "7d"; // JWT valid for 7 days

// TODO: Add email allowlist functionality
// For now, accept any email. Easy to add later with:
// const ALLOWED_EMAILS = (process.env.ADMIN_EMAILS || "").split(",").map(e => e.trim()).filter(Boolean);
// function isEmailAllowed(email) { return ALLOWED_EMAILS.includes(email); }
...
function createEmailTransporter() {
  ...
  if (!gmailPass) {
    console.warn("GMAIL_APP_PASSWORD not set. Email sending will fail.");
  }
...
export async function requestMagicLink(email) {
  ...
  // TODO: Add email allowlist check here when needed
