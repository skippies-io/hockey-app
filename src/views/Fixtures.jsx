...
export default function Fixtures({ ageId, ageLabel, ageGroups = [] }) {
  ...
  const labelForAge = (id) => ageLabelMap.get(id) || id || "Unknown age";
  ...
  useEffect(() => {
    ...
    return () => { alive = false; };
  }, [scopedAgeId, isAllAges, ageGroups]);
  ...
  }, [rows, date, onlyFollowing, isFollowing, scopedAgeId, isAllAges]);
  ...
  }, [filtered, isAllAges, ageOrder, ageLabelMap]);
  ...
  const renderCard = (f, idx, rowAgeId) => {
    ...
    const homeScore = hasScore(f.Score1) ? String(f.Score1).trim() : "";
    const awayScore = hasScore(f.Score2) ? String(f.Score2).trim() : "";
    let badge = null;
    if (played) {
      const a = Number(homeScore);
      const b = Number(awayScore);
    ...
    const metaHeader = joinMeta(f.Round);
    const venueLine = joinMeta(f.Venue, f.Pool || f.Group);

    return (
      <FixtureItem
        key={`${f.Date}-${f.Time}-${f.Team1}-${f.Team2}-${idx}-${followAgeId}`}
        mode={played ? "result" : "upcoming"}
        date={f.Date}
        time={played ? "" : f.Time}
        meta={metaHeader}
        venue={venueLine}
        homePrefix={
          <button
            className="star-btn"
            onClick={() => toggleFollow(followId1)}
        ...
        awayPrefix={
          <button
            className="star-btn"
            onClick={() => toggleFollow(followId2)}
        ...
        badge={badge}
        score={played ? `${homeScore} – ${awayScore}` : "TBD"}
        rightLabel={f.Time || "TBD"}
        followed={cardFollowed}
      />
    );
  };
